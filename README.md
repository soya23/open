...
その場で作れる(新フォルダ提案つき)。**「何をしたいか」を1行書くと、
役割チームはAIが設計**し、その目的が最初のタスクとしてボードに載る
(空Enterならテンプレート選択)。答え終わるとcockpitが開き、
全役割がそのまま仕事を始める。

パネルを増やしたい時も同じ思想: `a` を押して**やってほしいことを
書くだけ**(`テスト担当として壊れ方を探して`)。名前は自動で付き、
worktree作成→起動→設定保存まで勝手に済む。
...
### cockpit の操作 — 覚えるキーはゼロでいい

画面下に常にキーチップが並び、**`m` でメニュー**が開く: 全機能が日本語の
説明つきで一覧になり、`↑↓` と `Enter` だけで実行できる (zellij の
「見れば分かる」の徹底)。以下は直接キーで使いたい人向けの一覧:
...
- `a` — 役割を実行中に追加。**よく使う役割 (テスト/レビュー/ドキュメント/
  整理/企画) は選ぶだけ**、その他は「自由に書く」で文章から (名前は自動)
